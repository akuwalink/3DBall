...
object Collision{
            ...
            CollisionModels.COLLISION_MODE_BOX->{
                ...
                for (i in modelList) {
                    if(model!=i) {
                        ...
                        //dx = Math.abs(s_m.center.x - f_m.center.x)
    ...
    private fun defaultFun(model: Model,anModel: Model,mode:Int):Float{
        ...
        Log.e("now x y",model.collision_model.center.x.toString()+" "+model.collision_model.center.y+"   "+anModel.collision_model.center.x.toString()+" "+anModel.collision_model.center.y)
        return result
    }
